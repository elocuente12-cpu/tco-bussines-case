...
variable "imagebuilder_identifier" {
  type        = string
  description = "Identifier suffix for the Image Builder pipeline (e.g. windows-iis)."
}

variable "parent_image_ami" {
  type        = string
  description = "AMI ID of the Golden AMI (parent image) for the recipe."
}
...
variable "ami_share_account_ids" {
  type        = list(string)
  description = "AWS Account IDs to share the output AMI with (qa, stg, pro)."
  default     = []
}
